...
function UpdateFeed() {
  $.ajax({type:'GET', dataType:'xml', url: 'https://hn.algolia.com/rss/?sort=byPopularity&prefix&page=0&dateRange=last24h&type=story', timeout:5000, success:onRssSuccess, error:onRssError, async: false});
}
...
//	}
  ...
  for (var i=0; i< count; i++) {
    ...
    // Grab the Points
		var itemPoints = item.getElementsByTagName('points')[0];
		if (itemPoints) {
			hnLink.Points = itemPoints.textContent;
		} else {
			hnLink.Points = "0";
		}

    //Grab the Link
    var itemLink = item.getElementsByTagName('url')[0];
    if (itemLink) {
    ...
    var commentsLink = item.getElementsByTagName('comments')[0];
      ...
      hnLink.CommentsLink = '0';
    }

		var itemLink = item.getElementsByTagName('id')[0];
		var res = itemLink.textContent.replace("tag:hn.algolia.com,2005:Item/","https://news.ycombinator.com/item?id=")
		    if (res) {
					hnLink.ItemLink = res;
		} else {
			hnLink.ItemLink = '0';
